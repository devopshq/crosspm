...
Например, формат версий и столбцы для файла dependencies в конфиге вы определили так:
```yaml
columns: '*package, version, contract'
...
# packages                 version               contract
# --- DB:
db-config                  2.3.0.*               R.1.0
db-schema                  2.4.12.*              R.1.0
# --- BE:
backend3rdparty            2.4.*                 *
backend                    2.4.*                 tag_2
backend-doc                2.4.*                 *
chains                     1.0.*                 somevalue111
# --- UI:
#locales                   2.*                   tag_2
#ui                        2.*                   R.1.0
```
Как обычно, звёздочка заменяет поиск по любому значению свойства. В данном абстрактном примере артефакты базы должны быть совместимы с артефактом UI по билд-контракту contract=R.1.0. Сам контракт может быть установлен для артефакта в момент сборки, либо проставлен вручную позже.
