...
При выполнении команды `crosspm download` печатает сообщения в `stderr` (стандартный поток вывода ошибок). Из-за этого сложно понять правильно работает `crosspm` или там есть ошибки.
