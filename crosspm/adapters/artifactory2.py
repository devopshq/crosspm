...
class Adapter(artifactoryaql.Adapter):
    ...
    def convert_artifactory_path_packages_to_contracts_packages(self, artifactory_paths):
        result = []
        for p in artifactory_paths:
            contracts = []
            for key, val in p.properties.items():
                if key.startswith('c.'):
                    contracts += [(key, val[0])]

            result.append(crosspm.contracts.package.Package.create_package((p.name, p.version, contracts)))
    ...
    def get_packages_old(self, source, parser, downloader, list_or_file_path, property_validate=True):
        """
